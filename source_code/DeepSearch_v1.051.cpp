...
{
	//-------- header part -------//
	cerr << "DeepSearch v1.136 (pThread version) [Aug-29-2015] \n\n";
	...
	//-------- option part ------//
	cerr << "Usage: \n";
	cerr << "./DeepSearch -a NP -q query_file [-l data_list] [-d data_root] [-L ref_list] [-D ref_root] [-o out_file] \n";
...
int num_procs=1;
	...
	extern int optind;
	int c=0;
	while((c=getopt(argc,argv,"a:q:l:d:L:D:o:t:p:n:s:z:x:c"))!=EOF)
	{
		switch(c) 
		{
		case 'a':
