...
double **IN_ROT;
int IN_MAXLEN;
double *IN_RMSD_REC;  //-> for RMSD value output
int *IN_POS_REC;      //-> for RMSD value output
PDB_Residue **IN_ORIN;  //original input
PDB_Residue **IN_ORIN_TMP;
char **IN_IND;  //original index
...
void Init_Struc(int maxnum,int maxlen)
{
	//init
	IN_LEN=new int[maxnum];
	NewArray2D(&IN_MOL,maxnum,maxlen);
	NewArray2D(&IN_MOL_TMP,maxnum,maxlen);
	NewArray2D(&IN_AMI,maxnum,maxlen);
	NewArray2D(&IN_NAM,maxnum,maxlen);
	NewArray2D(&IN_ALI,maxnum,maxnum*maxlen);
	NewArray2D(&IN_ROT,maxnum,12);
	IN_RMSD_REC=new double[maxnum*maxlen];
	IN_POS_REC=new int[maxnum*maxlen];
}
int Return_Total(string &list)
{
	ifstream fin;
	string buf;
	//load
...
int Input_List(string &list,int &totnum,int limit_len)
{
	ifstream fin;
	string buf,temp;
	string name;
	...
	if(OUTPUT_TYPE>0)
	{
		IN_ORIN=new PDB_Residue*[totnum]; //original PDB_Residue
		IN_ORIN_TMP=new PDB_Residue*[totnum];
		IN_IND=new char*[totnum]; //original index
	}
	int cur=0;
	for(i=0;i<totnum;i++)
	{
		...
		ret_val=mol_input.XYZ_Input(file,range,0,IN_LEN[cur],0,0,0,0);
		if(ret_val<1)
		{
		...
		if(IN_LEN[cur]<HSFB_LEN_ORI)
		{
			fprintf(stderr,"%s -> the length %d of this protein is less than minimal %d!\n",
				buf.c_str(),IN_LEN[cur],HSFB_LEN_ORI);
			continue;
		}

		//record maximal
		if(IN_LEN[cur]>maxlen)maxlen=IN_LEN[cur];
		strcpy(IN_NAM[cur],buf.c_str());
		//input original
		if(OUTPUT_TYPE>0)
		{
			IN_ORIN[cur]=new PDB_Residue[IN_LEN[cur]];
			IN_ORIN_TMP[cur]=new PDB_Residue[IN_LEN[cur]];
			IN_IND[cur]=new char[IN_LEN[cur]*6];
			int PRE_LOAD_=mol_input.PRE_LOAD;
			int WARNING_out_=mol_input.WARNING_out;
			mol_input.PRE_LOAD=1;
			mol_input.WARNING_out=0;
			mol_input.XYZ_Input(file,range,0,moln,IN_MOL[cur],IN_AMI[cur],0,IN_IND[cur],IN_ORIN[cur]);
...
void BLOMAPS_Init(int maxnum,int maxlen)
{
...
//============= main_process =================//
void NEO_BLOMAPS(string &list,int totnum,string &outnam,string &outroot)
{
	Kabsch kabsch;
	//parameter
	int HSFB_LEN=12;
...
//	clock_t Time_Start=clock();
	double sumtms;
	sumtms=blomaps.BLOMAPS_Partial(totnum,IN_LEN,IN_MOL,IN_AMI,IN_ALI,IN_ROT);
//	clock_t Time_End=clock();
	...
	if(OUTPUT_TYPE>=2)
	{
		...
		{
			IN_MAXLEN=blomaps.FIN_BEST_LEN[k];
			for(int ii=0;ii<totnum;ii++)for(int jj=0;jj<IN_MAXLEN;jj++)IN_ALI[ii][jj]=blomaps.FIN_ALI_REC[k][ii][jj];
			vector <vector <int > > fin_ali_tmp (totnum, vector <int >(IN_MAXLEN));
			Transfer_Alignment(totnum,IN_MAXLEN,IN_ALI,fin_ali_tmp);
	...
	{
		if(OUTPUT_TYPE<2)if(k!=0)continue;
		int index=temp_index[indec_rec[k]];
		//superimpose CA structures
		IN_MAXLEN=blomaps.FIN_BEST_LEN[index];
		for(int ii=0;ii<totnum;ii++)for(int jj=0;jj<12;jj++)IN_ROT[ii][jj]=blomaps.FIN_ROT_REC[index][ii][jj];
		for(int ii=0;ii<totnum;ii++)for(int jj=0;jj<IN_MAXLEN;jj++)IN_ALI[ii][jj]=blomaps.FIN_ALI_REC[index][ii][jj];		
		for(int i=0;i<totnum;i++)blomaps.rot_mol(IN_MOL[i],IN_MOL_TMP[i],IN_LEN[i],IN_ROT[i]);
		//output multi solution
		if(OUTPUT_TYPE>0)
		{
			FILE *fp;
			char www_nam[300000];
			if(OUTPUT_TYPE>=2)sprintf(www_nam,"%s/%s.%d",outroot.c_str(),outnam.c_str(),k);
			else sprintf(www_nam,"%s/%s",outroot.c_str(),outnam.c_str());
			string rel_outnam=www_nam;
			//[0] superimpose original structure
			for(int i=0;i<totnum;i++)Superimpose_FullAtom(kabsch,IN_ORIN[i],IN_LEN[i],IN_ORIN_TMP[i],IN_ROT[i]); //original PDB
			//[1] output script
			string f2,f3;
			if(OUTPUT_CA>=1)
			{
				f3=rel_outnam+".scp";
				multi_out.BC_Output_All(f3,IN_MOL_TMP,IN_AMI,IN_LEN,totnum,IN_MAXLEN,IN_ALI,OUTPUT_CA);
			}
			//[2] output alignment
			...
			{
				Output_Alignment(fp,IN_AMI,IN_NAM,totnum,IN_MAXLEN,IN_ALI);
				fclose(fp);
			}
			//[3] output structure 
			string TER="TER                                                                             ";
			...
			else
			{
				for(int i=0;i<totnum;i++)
				{
					...
					mol_output.Output_PDB_III(fp,IN_LEN[i],IN_ORIN_TMP[i],chain,1);
					fprintf(fp,"%s\n",TER.c_str());
				}
				fprintf(fp,"%s\n",END.c_str());
				...
				if(Out_PDB==1)
				{
					...
					{
						char chain=Int_To_Dom(i);
						mol_output.Output_PDB_III(fq,IN_LEN[i],IN_ORIN_TMP[i],chain,-1);
						fprintf(fq,"%s\n",TER.c_str());
					}
					fprintf(fq,"%s\n",END.c_str());
					fclose(fq);
			...
			{
				Output_Rotmat(fp,IN_NAM,totnum,IN_ROT);
				fclose(fp);
			}
			//[5] output RMSD val
			blomaps.BC_Return_Conserved_Core_Pos(totnum,IN_MAXLEN,IN_ALI,IN_POS_REC);
			blomaps.BC_Return_Conserved_RMSD_Pos(totnum,IN_MAXLEN,IN_ALI,IN_MOL_TMP,IN_RMSD_REC);
			...
			else
			{
				...
				for(int i=0;i<IN_MAXLEN;i++)
				{
					char numc;
					if(IN_POS_REC[i]<totnum)numc=Int_To_Char(IN_POS_REC[i]);
					else numc='|';
					fprintf(fp,"%c",numc);
				}
				fprintf(fp,"\n");
			...
			//[6] output score
			double totsco=blomaps.BC_Calc_SumTM_Given_Mol(IN_MOL_TMP,IN_LEN,totnum,IN_MAXLEN,IN_ALI);
			int corelen=blomaps.BC_Return_Conserved_Core(totnum,IN_MAXLEN,IN_ALI);
			double totrms=blomaps.BC_Return_Conserved_RMSD(totnum,IN_MAXLEN,IN_ALI,IN_MOL_TMP);
...
int main(int argc,char **argv)
{
	...
	//---- macros -----//
	OUTPUT_CA=0;   //no script output
	REFINEMENT=0;  //no refinement
	OUTPUT_TYPE=1; //single solution
	...
	//-------- 3DCOMB -----------//__101110__//
	{
		...
		int input_num=Return_Total(input_name);
		if(input_num<2)
		{
			fprintf(stderr,"3DCOMB input file failed: too few number %d\n",input_num);
			exit(-1);
		}
		Init_Struc(input_num,PROT_MAX_NUM*2);
		int max_len=Input_List(input_name,input_num,PROT_MAX_NUM*2);
		if(max_len<0)
		{
		...
		//process
		BLOMAPS_Init(input_num,max_len);
		NEO_BLOMAPS(input_name,input_num,output_name,output_root);
		exit(0);
	}
}
